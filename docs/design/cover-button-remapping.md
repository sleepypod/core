...
- Left top/middle/bottom single clicks and top double-click are labeled and
  verified. Preserve the original structured `buttonEvent` count.
...
- Top+bottom overlap is observed in low-level edges, but bottom release is
  absent in the repeated trial. Complete combo duration/release is unproven.
...
Start with verified single/double presses. Expose holds and combinations
