...
describe('published HomeKit bridge persistence', () => {
  ...
  it('reopens HAP after advertiser failure even when its cleanup keeps rejecting', async () => {
    await startBridge(monitor)
    const first = bridge()
    const before = snapshot()
    const firstIds = ids(first)
    ...
    responders[0].shutdown.mockRejectedValue(new Error('mDNS unavailable'))

    await expect(unpairAll()).rejects.toThrow('mDNS unavailable')
